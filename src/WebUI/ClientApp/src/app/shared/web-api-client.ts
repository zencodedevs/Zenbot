...
export interface ICityClient {
    ...
    createJira(jiraId: string | null | undefined): Observable<void>;
}
...
export class CityClient implements ICityClient {
    ...
    createJira(jiraId: string | null | undefined): Observable<void> {
        let url_ = this.baseUrl + "/api/v1/City/CreateJira?";
        if (jiraId !== undefined && jiraId !== null)
            url_ += "jiraId=" + encodeURIComponent("" + jiraId) + "&";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processCreateJira(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processCreateJira(<any>response_);
                } catch (e) {
    ...
    protected processCreateJira(response: HttpResponseBase): Observable<void> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
